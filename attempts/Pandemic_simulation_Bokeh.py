#HOW TO EXECUTE IT IN CMD : bokeh serve --show Pandemic_simulation_Bokeh.py
...
class Visualiser:
	def __init__(self, world):
		self.world = world
		if world.nb_countries:
			self.fig = []
			self.traces = {}
			#self.fig = [[] for i in range(self.nb_rows)]
			for idx, c in world.countries.items():
				pos_x = idx % world.nb_cols
				pos_y = idx // world.nb_cols
				fig = figure(title='Country {}'.format(idx+1), output_backend="webgl", background_fill_color="lightgrey", x_range=(c.x1, c.x2), y_range=(c.y1, c.y2))
				fig.axis.visible, fig.xgrid.grid_line_color, fig.ygrid.grid_line_color = False, None, None
				#self.source[idx] = ColumnDataSource(data=dict(x=c.x_coord, y=c.y_coord, color=c.p_colors))
				self.traces[idx] = fig.scatter(x=c.x_coord, y=c.y_coord, fill_color=c.p_colors, line_color=c.p_colors, size=world.p_size) #color=c.p_colors
				self.fig.append(fig)
			self.grid = gridplot(self.fig, ncols=world.nb_cols, plot_width=250, plot_height=250)
			#p = gridplot(self.fig, plot_width=250, plot_height=250)
	...
	# 		self.traces[idx].data_source.data['x'] = c.x_coord
	# 		self.traces[idx].data_source.data['y'] = c.y_coord
	# 		self.traces[idx].data_source.data['fill_color'] = c.p_colors
	# 		self.traces[idx].data_source.data['line_color'] = c.p_colors
	# 	end = time.time()
...
curdoc().add_root(v.grid)

# Add a periodic callback to be run every 0 ms
curdoc().add_periodic_callback(v.update, 0)
