...
class Visualizer:
	def __init__(self, world):
		...
		#Graphe de l'état de l'épidémie
		subplot = self.fig[0,0]
		data_S = np.array([world.times, world.l_S]).T
		data_I = np.array([world.times, world.l_I]).T
		data_R = np.array([world.times, world.l_R]).T
		data_D = np.array([world.times, world.l_D]).T
		self.traces["S"] = subplot.plot(data_S, marker_size=0.0, face_color='b', edge_color='b')
		self.traces["I"] = subplot.plot(data_I, marker_size=0.0, face_color='b', edge_color='b')
		self.traces["R"] = subplot.plot(data_R, marker_size=0.0, face_color='b', edge_color='b')
		self.traces["D"] = subplot.plot(data_D, marker_size=0.0, face_color='b', edge_color='b')
		...
		for idx, c in world.countries.items():
			...
			subplot = self.fig[pos_y+1, pos_x]
	...
	def display(self):
		...
		data_S = np.array([self.world.times, self.world.l_S]).T
		data_I = np.array([self.world.times, self.world.l_I]).T
		data_R = np.array([self.world.times, self.world.l_R]).T
		data_D = np.array([self.world.times, self.world.l_D]).T
		self.traces["S"].set_data(data_S, marker_size=0.0, face_color='b', edge_color='b')
		self.traces["I"].set_data(data_I)
		self.traces["R"].set_data(data_R)
		self.traces["D"].set_data(data_D)
